...
Classify traffic-informing tweets into Good and Bad with class probability. Store in mongoDB
...
def save_classifier(classifier):
    global rf_classifier_filename
    f = open(rf_classifier_filename, 'wb')
    pickle.dump(classifier, f, -1)
    f.close()
    
def save_vectorizer(vectorizer):
    global rf_vectorizer_filename
...
def classify_tweets_relevance(threadName,output_collection,fetchedTweets):
    #Prepare the mongo lists for relevance classification
    try:
        counter = 0
        mongo_list = []
        sentences = []
...
def classify_mongo_list(mongo_list,sentences):
    ...
    #Classify each tweet in the mongo list and stores its relevance class and probability
    try:
        ...
        for i in range(0, len(mongo_list)):
            ...
            fetchedTweet['relevance_class'] = predictions[i]
    ...
    try:
        ...
        if len(mongo_list) > 0:
            for fetchedTweet in mongo_list:
                ...
                doc['relevance_class'] = fetchedTweet['relevance_class']
...
def process_days(threadName,stream,output_collection,next_date,end_date):
    try:
        ...
        logging.info("{}: Started extracting traffic relevance ...".format(threadName))
        classify_tweets_relevance(threadName,output_collection,preprocessedTweets)
        logging.info("{}: Finished extracting traffic relevance ...".format(threadName))
...
def parallel_process_days(start_date_str,end_date_str):
    ...
    try:
        ...
        output_collection = establish_connection_optimum("uk_accounts_tweet_relevance")
        training_collection = establish_connection_optimum("RFTrainingTweets")
