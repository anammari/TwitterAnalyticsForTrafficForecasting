...
plotTrafficTimeLines <- function(startStr, endStr, trafficConcepts) {
  ...
  if (mongo.is.connected(mongo) == TRUE) {
    for (i in 1:length(trafficConcepts)) {
      ...
      tweets_count <-
        mongo.count(mongo,"Twitter.uk_accounts_traffic_hr_agg",query = queryList)
      if (tweets_count > 0) {
        #flog.info("Total records to be fetched for concept %s: %s",trafficConcepts[i], tweets_count)
        result <-
          mongo.find(mongo, "Twitter.uk_accounts_traffic_hr_agg", query = queryList)
