...
def refresh_bcc_collections_optimum(collection):
    if collection == 'ANPR':
        pushBccAnprToMongoFirstTime.push_bcc_data_to_mongo_optimum(collection)
    else:
        pushBccDataToMongoFirstTime.push_bcc_data_to_mongo_optimum(collection)
...
def push_data_to_optimum_collections():
    try:
        collections = get_collections_names_optimum("BCC_traffic")
    except Exception, e:
        print(e)
        sys.exit()
    try:
        ...
        for collection in bcc_collections:
            if str(collection) not in collections:
                new = True
                refresh_bcc_collections_optimum(str(collection))
    except Exception, e:
        print(e)
        sys.exit()
    try:
    ...
    try:
        for collection in collections:
            if str(collection) != 'system.indexes':
                col = establish_connection_optimum(str(collection))
                ...
                add_bcc_traffic_optimum(str(collection),records)
